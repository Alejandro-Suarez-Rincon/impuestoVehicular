...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "impuestovehicular-ear.ear");

        // Current directory is the root of the "impuestovehicular-web" project. Go up one level
        // and enter the "impuestovehicular-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../impuestovehicular-ejb/target/impuestovehicular-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "impuestovehicular-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/impuestovehicular-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "impuestovehicular-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("us.codezen.test");
